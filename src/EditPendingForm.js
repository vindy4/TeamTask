...
class EditPendingForm extends React.Component{
    ...
    handleSubmit(event){               
         ...
         this.props.EditPendingItem(this.state);                               
         event.preventDefault();          
    }
    
    render(){

        const {title,task,assTo,reason} =this.state;
        const isEnabled = ( (title.length >0) && (task.length)>0 && (assTo.length)> 0 && (reason.length)> 0 )? true:false;
        return(
         <div>                                    
            <form className="form-group" onSubmit={this.handleSubmit}>          
                    ...
                    <textarea 
                        ...
                        placeholder=" Add pending reason.."  
                        rows="3">
                    </textarea>  <br />
             ...
             <button  disabled={!isEnabled} type="submit" className="btn btn-primary left-submit-button" >Submit</button>                   
                  <button onClick={this.close} type="button" className="btn btn-warning" data-dismiss="modal">Close</button>
            </div>
...
export default EditPendingForm;